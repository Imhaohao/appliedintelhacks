...
export function PostTaskForm({
  ...
  // Listen for demo quickfill events from the landing page.
  useEffect(() => {
    if (!demoPromptEvent) return;
    const handler = (e: Event) => {
      setPrompt((e as CustomEvent<string>).detail);
    };
    window.addEventListener(demoPromptEvent, handler);
    return () => window.removeEventListener(demoPromptEvent, handler);
  }, [demoPromptEvent]);

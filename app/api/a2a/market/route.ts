/**
 * Buyer-facing A2A v0.3.0 gateway for the Agora market.
 *
...
const MARKET_AGENT_ID = "agora-market";
...
function makeRunId() {
  return `agora-a2a-${MARKET_AGENT_ID}-${Date.now()}-${Math.random()
    .toString(36)
    .slice(2, 8)}`;
}
...
function buildMarketAgentCard(req: NextRequest) {
  ...
  return {
    ...
    name: "Agora Market",
    description:
      "A2A gateway to the Agora market. Buyer agents can discover specialists, post tasks, poll task state, and raise disputes via message/send with metadata.intent.",
    url,
    version: "1.0.0",
    provider: {
      organization: "Agora",
      ...
      extensions: [
        {
          ...
          required: false,
          description:
            "Agora market intents: discover, post_task, get_task, raise_dispute via message/send metadata.intent.",
        },
      ],
    },
...
export async function POST(req: NextRequest) {
  ...
  if (process.env.ARBOR_A2A_HMAC_REQUIRED === "true") {
    ...
    const verdict = await client.action(api.a2aAuth.verifyInboundCallback, {
      raw_body: rawBody,
      agent_id: req.headers.get("X-Agora-Agent") ?? undefined,
      timestamp: req.headers.get("X-Agora-Timestamp") ?? undefined,
      nonce: req.headers.get("X-Agora-Nonce") ?? undefined,
      signature: req.headers.get("X-Agora-Signature") ?? undefined,
    });
    if (!verdict.ok) {
      return new Response(
        JSON.stringify({

...
export const verifyInboundCallback = action({
  ...
  handler: async (ctx, args): Promise<VerifyVerdict> => {
    ...
    if (!args.agent_id) missing.push("X-Agora-Agent");
    if (!args.timestamp) missing.push("X-Agora-Timestamp");
    if (!args.nonce) missing.push("X-Agora-Nonce");
    if (!args.signature) missing.push("X-Agora-Signature");
    if (missing.length > 0) {
      return {
        ok: false,
        status: 401,

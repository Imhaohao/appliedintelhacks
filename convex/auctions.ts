...
export const solicitBids = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    // Open auction: every registered specialist gets a chance to bid.
    const invitedSpecialists = [...SPECIALISTS, ...discoveredConfigs, ...catalogConfigs];
...
export const settle = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    if (verdict.verdict === "accept") {
      // ─── Opsera security gate ───────────────────────────────────────────────
      // Scan code-producing deliverables before releasing escrow.
      const resultText: string =
        typeof task.result?.text === "string" ? task.result.text : "";
      const prUrl: string | undefined =
        task.result?.artifact?.pr_url ??
        task.result?.provenance?.pr_url ??
        undefined;
      const isCodeDeliverable =
        !!prUrl ||
        (resultText.length > 100 &&
          (resultText.includes("```") ||
            resultText.includes("function ") ||
            resultText.includes("SELECT ") ||
            resultText.includes("import ")));

      if (isCodeDeliverable && process.env.OPSERA_API_KEY) {
        const { scanDeliverable } = await import("../lib/opsera-scan");
        const scanResult = await scanDeliverable({
          code: resultText || undefined,
          pr_url: prUrl,
          task_id: args.task_id,
        }).catch((err) => ({
          passed: true as const,
          findings: [{ severity: "info" as const, type: "scan_error", message: String(err) }],
          severitySummary: { critical: 0, high: 0, medium: 0, low: 0, info: 1 },
          scan_duration_ms: 0,
          endpoint_used: "",
          skipped: true,
        ...
        await ctx.runMutation(internal.tasks._setScanResult, {
          task_id: args.task_id,
        ...
        await ctx.runMutation(internal.lifecycle.log, {
          ...
          payload: {
            ...
            severitySummary: scanResult.severitySummary,
            findings_count: scanResult.findings.length,
            skipped: scanResult.skipped ?? false,
          },
        });

        // Upload scan result to Tigris for audit trail.
        try {
          const { putScanResult } = await import("../lib/tigris");
          await putScanResult(args.task_id, scanResult);
        } catch { /* non-fatal */ }

        if (!scanResult.passed) {
          // Block escrow — hold funds, penalize specialist.
          await ctx.runMutation(internal.escrow._settle, {
            task_id: args.task_id,
            status: "refunded",
          });
          ...
          await ctx.runMutation(internal.agents._applyReputationDelta, {
            ...
            reasoning: `Opsera security scan failed: ${scanResult.severitySummary.critical} critical, ${scanResult.severitySummary.high} high severity findings.`,
          ...
          await ctx.runMutation(internal.lifecycle.log, {
            ...
            event_type: "settled",
            payload: {
              verdict: "security_hold",
              escrow: "refunded",
              seller_id: winner.agent_id,
              delta: secDelta,
              critical_findings: scanResult.severitySummary.critical,
              high_findings: scanResult.severitySummary.high,
            },
          });
          // Hand back to planner if this was a subtask.
      ...
      // Upload judge-verified artifact to Tigris.
      try {
        const { putJudgedArtifact, snapshotWorldState } = await import("../lib/tigris");
        const artifactResult = await putJudgedArtifact(args.task_id, task.result);
        if (artifactResult) {
          await ctx.runMutation(internal.tasks._setTigrisArtifact, {
            task_id: args.task_id,
            artifact_tigris_key: artifactResult.key,
            artifact_tigris_url: artifactResult.url,
          });
        }
        // World-state snapshot for replay.
        const snapResult = await snapshotWorldState(args.task_id, {
          task_id: args.task_id,
          prompt: task.prompt,
        ...
        if (snapResult) {
          await ctx.runMutation(internal.tasks._setWorldState, {
            task_id: args.task_id,
            world_state_tigris_key: snapResult.key,
          });
        }
      } catch (err) {
        // Non-fatal — Tigris failure must not block escrow release.
        console.error("[tigris] artifact upload failed:", err);
      }

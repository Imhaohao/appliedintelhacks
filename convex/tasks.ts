    ...
    if (profile) {
      await ctx.runMutation(internal.lifecycle.log, {
        ...
        payload: {
          ...
          tigris_context_key: profile.tigris_context_key,
        },
      });
    }
...
export const _setScanResult = internalMutation({
  args: { task_id: v.id("tasks"), scan_result: v.any() },
  handler: async (ctx, args) => {
    await ctx.db.patch(args.task_id, { scan_result: args.scan_result });
  },
});

export const _setTigrisArtifact = internalMutation({
  args: {
    task_id: v.id("tasks"),
    artifact_tigris_key: v.string(),
    artifact_tigris_url: v.string(),
  },
  handler: async (ctx, args) => {
    await ctx.db.patch(args.task_id, {
      artifact_tigris_key: args.artifact_tigris_key,
      artifact_tigris_url: args.artifact_tigris_url,
    });
  },
});

export const _setWorldState = internalMutation({
  args: { task_id: v.id("tasks"), world_state_tigris_key: v.string() },
  handler: async (ctx, args) => {
    await ctx.db.patch(args.task_id, {
      world_state_tigris_key: args.world_state_tigris_key,
    });
  },
});

...
export default defineSchema({
  ...
  tasks: defineTable({
    ...
    /** Opsera security scan result — present on code-producing tasks. */
    scan_result: v.optional(v.any()),
    /** Tigris artifact storage pointers. */
    artifact_tigris_key: v.optional(v.string()),
    artifact_tigris_url: v.optional(v.string()),
    world_state_tigris_key: v.optional(v.string()),
    /**
  ...
  product_context_profiles: defineTable({
    ...
    /** Tigris key for the persisted context blob (replaces hyperspell_status). */
    tigris_context_key: v.optional(v.string()),
    created_at: v.number(),
    updated_at: v.number(),
  ...
  // Per-agent shared secret for HMAC signing & replay protection.
  ...
  // Nonces seen on signed inbound callbacks — replay protection.
  ...
  /**
   * Per-task multi-dimensional performance record.
   */
